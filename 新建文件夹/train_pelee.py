...
def train(args, data_root, save_root):
    ...
    log_dir = "{}logs/ScratchPeleeNet-{}".format(save_root, time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()))
    ...
    model = PeleeNet(num_classes=n_classes, in_size=(net_h, net_w))
    ...
    if args.tensor_board:
        writer = SummaryWriter(log_dir=log_dir, comment="ScratchPeleeNet")
    ...
    for epoch in np.arange(args.start_epoch, args.n_epoch):
        ...
        with torch.no_grad():
            ...
            if class_iou["Class#1"] >= best_iou:
                ...
                torch.save(state, "{}{}_peleenet_best_model.pkl".format(weight_dir, args.dataset))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--l_rate', nargs='?', type=float, default=7.5e-3,
                        help='Learning Rate')
    parser.add_argument('--resume', nargs='?', type=str, default="scratch_peleenet_best_model.pkl",
